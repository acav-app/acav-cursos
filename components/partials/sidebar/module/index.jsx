...
import { filterMenusByRole, menusConfig } from "@/config/menus";
...
const ModuleSidebar = ({ trans }) => {
  ...
  const toggleSubMenu = (index) => {
    ...
    if (iconMenus[index]?.child) {
      setCurrentSubMenu(iconMenus[index].child);
      setSubmenu(false);
      setCollapsed(false);
      if (!isDesktop) {
        setMenuOverlay(true);
      ...
      if (!isDesktop) {
        ...
        if (isLocationMatch(iconMenus[index]?.href, locationName)) {
          setSubmenu(false);
        }
      }
    }
  ...
  const getMenuTitle = () => {
    if (activeIndex !== null) {
      return iconMenus[activeIndex]?.title || "";
    }
    return "";
  };
  ...
  }, [iconMenus, locationName, isDesktop, setCollapsed, setSubmenu]);
