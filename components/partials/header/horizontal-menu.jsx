      ...
      >
        ...
        >
          {menus?.map((item, index) => {
            const hasDropdown = Boolean(item?.child?.length) || Boolean(item?.megaMenu?.length);
            if (!hasDropdown) {
              return (
                <NavigationMenu.Item key={`item-${index}`}>
                  <NavigationMenu.Link asChild>
                    <Link
                      ...
                      className="flex items-center py-4 cursor-pointer transition-colors hover:text-primary"
                    >
