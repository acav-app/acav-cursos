...
function resolveSocialMeta(link) {
  ...
  if (haystack.includes("instagram")) {
    return { icon: "ri:instagram-fill", hoverClass: "hover:border-[#E4405F] hover:bg-[#E4405F]" };
  }

  if (haystack.includes("linkedin")) {
    return { icon: "ri:linkedin-fill", hoverClass: "hover:border-[#0A66C2] hover:bg-[#0A66C2]" };
  }

  if (haystack.includes("facebook")) {
    return { icon: "ri:facebook-fill", hoverClass: "hover:border-[#1877F2] hover:bg-[#1877F2]" };
  }

  if (haystack.includes("youtube")) {
    return { icon: "ri:youtube-fill", hoverClass: "hover:border-[#FF0000] hover:bg-[#FF0000]" };
  }

  if (haystack.includes("tiktok")) {
    return { icon: "ri:tiktok-fill", hoverClass: "hover:border-[#111111] hover:bg-[#111111]" };
  }

  if (haystack.includes("whatsapp")) {
    return { icon: "ri:whatsapp-fill", hoverClass: "hover:border-[#25D366] hover:bg-[#25D366]" };
  }

  if (haystack.includes("twitter") || haystack.includes("x.com") || label === "x" || haystack.includes(" x ")) {
    return { icon: "ri:twitter-x-fill", hoverClass: "hover:border-white hover:bg-white hover:text-[#15203B]" };
  }

  return { icon: "ri:global-line", hoverClass: "hover:border-[#31456F] hover:bg-[#31456F]" };
}

export default function PublicCoursesShell({ lang, settings, navMode = "routes", children }) {
  const catalogoHref = navMode === "anchors" ? "#catalogo" : `/${lang}/cursos#catalogo`;
  const porqueAcavHref = navMode === "anchors" ? "#porque-acav" : `/${lang}/cursos#porque-acav`;
  const testimoniosHref = navMode === "anchors" ? "#testimonios" : `/${lang}/cursos#testimonios`;
  ...
  const accountHomeHref = isDashboardRole ? `/${lang}/dashboard` : user ? candidateAreaHref : `/${lang}/auth/login`;
  const displayName = actor?.firstName || actor?.displayName || user?.displayName || user?.email?.split("@")[0] || "Usuario";
  const avatarLetter = String(displayName || "U").trim().charAt(0).toUpperCase() || "U";
  const socialLinks = Array.isArray(settings?.socialLinks) && settings.socialLinks.length ? settings.socialLinks : FALLBACK_SOCIALS;
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#15203B]/92 backdrop-blur-xl">
        <div className="mx-auto flex h-[68px] max-w-[1180px] items-center justify-between gap-6 px-6">
          <Link href={`/${lang}`} className="flex items-center gap-3">
            <SiteLogo withBg={false} className="h-9 w-auto drop-shadow-[0_2px_10px_rgba(0,0,0,0.35)]" />
            <span className="h-6 w-px bg-white/20" />
            <span className="text-sm font-extrabold uppercase tracking-[0.08em] text-[#DD4913]">Cursos</span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            {isAccountResolving ? (
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    <span className="max-w-[132px] truncate text-sm font-semibold">{displayName}</span>
                    <ChevronDown className="h-4 w-4 text-white/65" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-64 rounded-2xl border border-slate-200 p-2">
                  <DropdownMenuLabel className="px-3 py-2">
                    <div className="text-sm font-semibold text-slate-900">{displayName}</div>
                    <div className="mt-1 text-xs text-slate-500">{actor?.email || user?.email || ""}</div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem className="rounded-xl px-3 py-2 text-red-600 focus:text-red-600" onSelect={handleLogout}>
              ...
              <Button asChild variant="outline" className="hidden rounded-full border-white/20 bg-white/5 px-4 text-white hover:bg-white/10 hover:text-white md:inline-flex">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-60 rounded-2xl border border-slate-200 p-2">
                  <DropdownMenuLabel className="px-3 py-2">
                    <div className="text-sm font-semibold text-slate-900">{displayName}</div>
      ...
      <footer className="bg-[#15203B] text-white/65">
        <div className="mx-auto max-w-[1180px] px-6 py-16">
          <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[2fr_1fr_1fr_1.3fr]">
            <div>
              <SiteLogo withBg={false} className="h-10 w-auto drop-shadow-[0_2px_10px_rgba(0,0,0,0.35)]" />
              ...
              <p className="mt-4 max-w-md text-sm leading-7">
                La plataforma de formacion virtual de la Asociacion Cordobesa de Agencias de Viajes. Centraliza cursos,
                certificaciones e inscripciones para el ecosistema turistico.
              </p>
              <div className="mt-6 flex gap-3">
                {socialLinks.map((social, index) => {
                  ...
                  return social.url ? (
                    <a key={`${social.label}-${index}`} href={social.url} target="_blank" rel="noreferrer">
                      {content}
                    </a>
                  ) : (
            ...
            <div>
              <div className="text-xs font-extrabold uppercase tracking-[0.18em] text-white">Cursos</div>
              <div className="mt-5 flex flex-col gap-3 text-sm">
                <Link href={`/${lang}/cursos`} className="transition hover:text-white">
                  Amadeus Básico
                </Link>
                <Link href={`/${lang}/cursos`} className="transition hover:text-white">
                  Amadeus Avanzado
            ...
            <div>
              <div className="text-xs font-extrabold uppercase tracking-[0.18em] text-white">Plataforma</div>
              <div className="mt-5 flex flex-col gap-3 text-sm">
                <Link href={accountHomeHref} className="transition hover:text-white">
                  {isDashboardRole ? "Campus admin" : user ? "Mi campus" : "Acceder"}
                </Link>
                <ViewTransitionLink href={`/${lang}/mis-cursos`} className="transition hover:text-white">
                  Mis cursos
                </ViewTransitionLink>
                <ViewTransitionLink href={`/${lang}/mis-certificados`} className="transition hover:text-white">
                  Mis certificados
                </ViewTransitionLink>
                <ViewTransitionLink href={`/${lang}/historial-pagos`} className="transition hover:text-white">
                  Historial de pagos
                </ViewTransitionLink>
                <Link href="https://www.acav.com.ar" className="transition hover:text-white">
                  Ir a acav.com.ar →
                </Link>
              </div>
            </div>
            ...
            <div>
              <div className="text-xs font-extrabold uppercase tracking-[0.18em] text-white">Contacto</div>
              <div className="mt-5 grid gap-3 text-sm leading-6">
                <div className="flex gap-3">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-white/75" />
                  <span>{settings?.address || "Obispo Salguero 169 1º Piso, Of. 1 y 2 — Córdoba, Argentina"}</span>
                </div>
                <div className="flex gap-3">
          ...
          <div className="mt-8 flex flex-col gap-3 text-xs md:flex-row md:items-center md:justify-between">
            <span>© 2026 ACAV · Asociación Cordobesa de Agencias de Viajes. Todos los derechos reservados.</span>
            <div className="flex flex-wrap items-center gap-3 md:justify-end">
              ...
              <a href="https://www.acav.com.ar" target="_blank" rel="noreferrer" className="transition hover:text-white">
                Ir a acav.com.ar →
              </a>
            </div>
          </div>
