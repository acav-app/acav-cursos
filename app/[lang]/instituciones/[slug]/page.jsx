...
export default async function InstitucionDetailPage({ params: { lang, slug } }) {
  const [settings, institution] = await Promise.all([getPublicCourseSettings(), getPublicInstitutionBySlug(slug)]);

  if (!institution) {
    notFound();
  }

  const institutionCourses = (await getPublicCourses({ companyId: institution.id })).map(mapCourseForCard);
  const coverUrl = institution?.coverUrl ? normalizePublicR2Url(institution.coverUrl) : "";
  const logoUrl = institution?.logoUrl ? normalizePublicR2Url(institution.logoUrl) : "";
            ...
            <MotionReveal className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
              {coverUrl ? (
                <div className="h-56 w-full overflow-hidden border-b border-slate-200 bg-slate-100">
                  <img src={coverUrl} alt={`Portada de ${institution.name}`} className="h-full w-full object-cover transition duration-[1600ms] hover:scale-[1.04]" />
                </div>
              ) : (
              ...
              <div className="grid gap-8 px-6 py-8 lg:grid-cols-[1.4fr_.6fr]">
                <div className="flex items-start gap-5">
                  <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                    {logoUrl ? (
                      <img src={logoUrl} alt={`Logo de ${institution.name}`} className="h-full w-full object-contain p-3" />
                    ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <h1 className="mt-2 text-4xl font-black text-[#1B2B50]">{institution.name}</h1>
                    <div className="mt-3 flex flex-wrap gap-3 text-sm text-slate-500">
                      {institution?.city ? (
                        <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2">
                          ...
                          <span>{[institution.city, institution.province].filter(Boolean).join(", ")}</span>
                        </div>
                      ) : null}
                      {institution?.website ? (
                        <a
                    ...
                    <p className="mt-5 max-w-3xl text-base leading-8 text-slate-600">
                      {institution.description || "La institucion todavia no completo su descripcion publica."}
                    </p>
                  </div>
                </div>
                ...
                <aside className="rounded-[22px] border border-slate-200 bg-slate-50 p-6">
                  ...
                  <div className="mt-4 grid gap-3 text-sm text-slate-600">
                    <div>
                      ...
                      <div>{institution?.subRubro || "No informado"}</div>
                    </div>
                    <div>
                      <div className="font-semibold text-slate-900">Ciudad / provincia</div>
                      <div>{[institution?.city, institution?.province].filter(Boolean).join(" / ") || "No informado"}</div>
                    </div>
                    <div>
                      <div className="font-semibold text-slate-900">Cursos activos</div>
                      <div>{institutionCourses.length}</div>
                    </div>
                  </div>
                </aside>
              </div>
          ...
          {institutionCourses.length ? (
            <MotionStagger className="mt-6 grid gap-4">
              {institutionCourses.map((course) => {
                const previewUrl = normalizePublicR2Url(course.imageUrl || course.flyerUrl || institution.logoUrl || "");
                return (
                        ...
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                          <div className="flex min-w-0 items-center gap-4">
                            <div className="h-20 w-24 shrink-0 overflow-hidden rounded-2xl bg-slate-100">
                              {previewUrl ? (
                                <img src={previewUrl} alt={course.title} className="h-full w-full object-cover transition duration-700 hover:scale-105" />
                              ) : (
                                <div className="flex h-full w-full items-center justify-center text-xs font-bold text-slate-500">ACAV</div>
                              )}
                            </div>
                            <div className="min-w-0">
                              <Link href={`/${lang}/cursos/${course.slug}`} className="block">
                                <h3 className="text-lg font-bold text-[#1B2B50] transition hover:text-[#31456F]">{course.title}</h3>
                              </Link>
                              <p className="text-sm text-slate-500">
                                {course.city} · {course.modalidad} · {course.contractType}
                              </p>
                            </div>
                          </div>
                          <div className="flex flex-wrap items-center gap-3">
                            <Link href={`/${lang}/cursos/${course.slug}`} className="text-sm font-semibold text-[#31456F] transition hover:text-[#1B2B50]">
                              Ver curso
                            </Link>
                        <CourseEnrollButton
                              lang={lang}
