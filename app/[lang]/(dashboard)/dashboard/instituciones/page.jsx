...
export default function DashboardInstitucionesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!alive) return;
        setInstitutions(asArray(data?.institutions));
      } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    return institutions.filter((institution) =>
      [institution.name, institution.city, institution.subRubro, institution.slug].some((value) =>
        ...
        <div className="mt-6 grid gap-4">
          {filtered.length ? (
            filtered.map((institution) => (
              <Link
                key={institution.id}
                href={buildLocalizedPath(`/dashboard/instituciones/${institution.id}`)}
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div className="flex min-w-0 items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-lg font-semibold text-foreground">{institution.name}</div>
                      <div className="text-sm text-muted-foreground">
                        {institution.city || "Sin ciudad"} · {institution.subRubro || "Sin rubro"} · {institution.status}
                      </div>
                    </div>
                  </div>
                  <div>
