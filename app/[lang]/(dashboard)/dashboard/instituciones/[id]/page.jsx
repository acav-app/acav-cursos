...
export default function InstitucionEditPage({ params: { id } }) {
  ...
  const isInstitutionActor = actor?.role === "empresa";
  ...
  const initials = String(institution?.name || "IN").slice(0, 2).toUpperCase();
    ...
    <div className="py-8 px-2 max-w-5xl mx-auto">
      ...
      <div className="rounded-3xl border border-border/60 bg-card p-8">
        <div className="overflow-hidden rounded-[28px] border border-border/60 bg-background">
          {coverUrl ? (
            <div className="h-48 w-full overflow-hidden border-b border-border/60 bg-muted/20">
              <img src={coverUrl} alt={institution?.name || "Portada de institucion"} className="h-full w-full object-cover" />
            </div>
          ) : (
          ...
          <div className="flex flex-col gap-4 p-6 md:flex-row md:items-center">
            <Avatar className="h-24 w-24 rounded-[28px] border border-border/60 bg-white">
              {logoUrl ? <AvatarImage src={logoUrl} alt={institution?.name || "Logo de institucion"} className="object-contain bg-white p-2" /> : null}
              <AvatarFallback className="rounded-[28px] bg-primary/10 text-xl font-semibold text-primary">
                {initials}
              </AvatarFallback>
            </Avatar>
            <div className="min-w-0 flex-1">
              <div className="text-2xl font-bold text-foreground">{institution?.name || (isInstitutionActor ? "Mi institucion" : "Institucion")}</div>
              <div className="mt-2 text-sm text-muted-foreground">
                {[institution?.city, institution?.province, institution?.subRubro].filter(Boolean).join(" · ") || "Completa la ficha publica de tu institucion."}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="text-sm font-semibold uppercase tracking-[0.2em] text-primary">
          {isInstitutionActor ? "Mi institucion" : "Instituciones"}
        </div>
        <h1 className="mt-3 text-3xl font-bold text-foreground">
          {isInstitutionActor ? "Gestiona tu institucion" : "Editar institucion"}
        </h1>
